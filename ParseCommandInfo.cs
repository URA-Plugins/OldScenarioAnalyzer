    ...
    public static class Analyzer
    {
        public static async void ParseCommandInfo(Gallop.SingleModeCheckEventResponse @event)
        {
            ...
            if (@event.IsScenario(ScenarioType.LArc))
            {
                ...
                //{
                //    var tips = FromSkillTipsResponseAnalyzer.CalculateSkillScoreCost(@event, Database.Skills.Apply(@event.data.chara_info), false);
                //
            ...
            foreach (var command in @event.data.home_info.command_info_array)
            {
                ...
                var partners = command.training_partner_array
                    .Select(partner =>
                    {
                        ...
                        var isArcPartner = @event.IsScenario(ScenarioType.LArc) && (partner > 1000 || (partner >= 1 && partner <= 7)) && @event.data.arc_data_set.evaluation_info_array.Any(x => x.target_id == partner);
